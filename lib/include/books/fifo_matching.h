...
namespace ff::books {
    ...
    public:
        std::vector<Trade> match(
            Order& aggressive_order, PriceOrderBook& passive_book,
            PriceQuantityMap& cumulative_volume) {
            std::vector<Trade> trades;
            trades.reserve(1e5);
            // need to compare the top of the book
            while (aggressive_order.qty > 0 && !passive_book.empty() &&
                   compare_with_passive(
                       aggressive_order, passive_book.begin()->first)) {
                ...
                auto traded_quantity =
                    std::min(aggressive_order.qty, next_order.qty);

                trades.emplace_back(
                    aggressive_order.order_id, next_order.order_id,
                    current_price, traded_quantity);
    ...
    private:
        static constexpr auto compare_with_passive =
            [](const Order& aggressive_order, const Price passive_price) {
